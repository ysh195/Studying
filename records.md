...
4. 지뢰찾기
   ...
   - 버튼은 지뢰인지 아닌지, 혹은 자신의 위치 좌표 등의 정보를 가지고 있어야 하기 때문에 반드시 생성자로 만들어야 한다고 생각했음.
   - 근데 그렇게 하면 배열이 안 되니까 차라리 버튼, 지뢰 여부, 좌표를 따로 만들어서 배열의 위치 정보가 일치하면 그것의 정보임을 아는 방법으로 시도도 해봄
   - 그랬더니 결국 버튼부터가 배열로 만드는 게 불가능하고, 물론 안 됐지만 버튼으로 만든다고 해도 버튼 자체도 그 위치 정보를 출력하도록 설정하는 게 문제
